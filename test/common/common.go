...
func VerifyRegistryCache(parentCtx context.Context, log logr.Logger, shootClient kubernetes.Interface, image string, mutateFns ...MutatePodFn) {
	...
	EventuallyWithOffset(1, ctx, func() error {
		...
		output, err := rootPodExecutor.Execute(ctx, "sh", "-c", fmt.Sprintf(jqExtractRegistryLocation, containerID))
		if err != nil {
		...
		output, err = rootPodExecutor.Execute(ctx, "cat", fmt.Sprintf(`%s/docker/registry/v2/repositories/%s/_manifests/tags/%s/current/link`, registryRootPath, path, tag))
		if err != nil {
		...
		output, err = rootPodExecutor.Execute(ctx, "sh", "-c", fmt.Sprintf(jqExtractManifestDigest, arch, registryRootPath, imageIndexPath))
		if err != nil {
		...
		output, err = rootPodExecutor.Execute(ctx, "sh", "-c", fmt.Sprintf(jqExtractLayersDigests, registryRootPath, manifestPath))
		if err != nil {
		...
		for _, layerDigest := range layerDigests {
			_, err = rootPodExecutor.Execute(ctx, "ls", fmt.Sprintf(`%s/docker/registry/v2/blobs/%s/data`, registryRootPath, sha256Path(layerDigest)))
			if err != nil {
